...
uint16_t addr_imm(CPU *cpu, BUS *bus, uint8_t *page_cross)     //Immediate
{
    return cpu->pc++;
}
uint16_t addr_zp(CPU *cpu, BUS *bus, uint8_t *page_cross)      // Zeropage
{
    return (uint16_t)bus_read(bus,cpu->pc++); // no hi byte, so just return this as uint16_t - will make highbyte zero
}
uint16_t addr_zpx(CPU *cpu, BUS *bus, uint8_t *page_cross)     // Zeropage, X
{
    return ((uint16_t)(bus_read(bus, cpu->pc++)) + (uint16_t)(cpu->x)) & 0x00FF; //page wrapping in zero page -> no cycle penalty
}
uint16_t addr_zpy(CPU *cpu, BUS *bus, uint8_t *page_cross)     // Zeropage, Y
{
    return ((uint16_t)(bus_read(bus, cpu->pc++)) + (uint16_t)(cpu->y)) & 0x00FF;
    //return (uint16_t)((bus_read(bus, cpu->pc++) + cpu->y) & 0xFF); //page wrapping in zero page -> no cycle penalty
}
uint16_t addr_abs(CPU *cpu, BUS *bus, uint8_t *page_cross)     // Absolute
{
    uint16_t lo = (uint16_t)bus_read(bus, cpu->pc++);
    uint16_t hi = (uint16_t)bus_read(bus, cpu->pc++);
    return (hi << 8) | lo;  
}
uint16_t addr_absx(CPU *cpu, BUS *bus, uint8_t *page_cross)    // Absolute,X
{
    uint16_t lo = (uint16_t)bus_read(bus, cpu->pc++);
    uint16_t hi = (uint16_t)bus_read(bus, cpu->pc++);
...
uint16_t addr_absy(CPU *cpu, BUS *bus, uint8_t *page_cross)    // Absolute,Y
{
    uint16_t lo = (uint16_t)bus_read(bus, cpu->pc++);
    uint16_t hi = (uint16_t)bus_read(bus, cpu->pc++);
...
uint16_t addr_indirx(CPU *cpu, BUS *bus, uint8_t *page_cross)  // Indirect, X
{
    uint16_t zp_addr = ((uint16_t)(bus_read(bus, cpu->pc++)) + (uint16_t)(cpu->x)) & 0x00FF;
    uint16_t lo = (uint16_t)bus_read(bus, zp_addr);
    uint16_t hi = (uint16_t)(bus_read(bus, (zp_addr + 1) & 0x00FF));
    return (hi << 8) | lo;  
}
uint16_t addr_indiry(CPU *cpu, BUS *bus, uint8_t *page_cross)  // Indirect, Y
{
    uint16_t zp_addr = (uint16_t)(bus_read(bus, cpu->pc++)) & 0x00FF;
...
uint16_t addr_rel(CPU *cpu, BUS *bus, uint8_t *page_cross)     // Relative
{
    int8_t offset = (int8_t)bus_read(bus,cpu->pc++);
    return (uint16_t)(cpu->pc + offset);
}
uint16_t addr_imp(CPU *cpu, BUS *bus, uint8_t *page_cross)     // Implied - return dummy address
{
    return 0x0000;
}
uint16_t addr_indir(CPU *cpu, BUS *bus, uint8_t *page_cross)   // Indirect Jump Resolution + bug
{
